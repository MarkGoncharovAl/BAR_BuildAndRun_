...
std::array<sfC::Picture_t , sfDATA::MainMenu_t::count_buttons_> sfDATA::MainMenu_t::buttons_ = {
    ...
    sfC::Picture_t ("../Images/buttons2.png", sfDATA::MainMenu_t::released_, sf::Vector2f (sfCON::WindowWidth / 3, sfCON::WindowHeight / 10 + 150), sf::Color (255, 255, 255), sf::Vector2f (1.2f, 1.2f)),
    sfC::Picture_t ("../Images/buttons2.png", sfDATA::MainMenu_t::released_, sf::Vector2f (sfCON::WindowWidth / 3, sfCON::WindowHeight / 10 + 300), sf::Color (255, 255, 255), sf::Vector2f (1.2f, 1.2f)),
    sfC::Picture_t ("../Images/buttons2.png", sfDATA::MainMenu_t::released_, sf::Vector2f (sfCON::WindowWidth / 3, sfCON::WindowHeight / 10 + 450), sf::Color (255, 255, 255), sf::Vector2f (1.2f, 1.2f)),
    sfC::Picture_t ("../Images/buttons2.png", sfDATA::MainMenu_t::released_, sf::Vector2f (sfCON::WindowWidth / 3, sfCON::WindowHeight / 10 + 600), sf::Color (255, 255, 255), sf::Vector2f (1.2f, 1.2f)),
    sfC::Picture_t ("../Images/buttons2.png", sfDATA::MainMenu_t::released_, sf::Vector2f (sfCON::WindowWidth / 3, sfCON::WindowHeight / 10 + 750), sf::Color (255, 255, 255), sf::Vector2f (1.2f, 1.2f)) };
...
std::array<sfC::Text_t , sfDATA::MainMenu_t::count_buttons_> sfDATA::MainMenu_t::texts_ = {
    ...
    sfC::Text_t ("../Fonts/baltimore-nouveau.ttf", 28, sf::Vector2f (sfCON::WindowWidth / 3 + 27, sfCON::WindowHeight / 10 + 174), "Network"),
    sfC::Text_t ("../Fonts/baltimore-nouveau.ttf", 22, sf::Vector2f (sfCON::WindowWidth / 3 + 46, sfCON::WindowHeight / 10 + 328), "Restart"),
    sfC::Text_t ("../Fonts/baltimore-nouveau.ttf", 22, sf::Vector2f (sfCON::WindowWidth / 3 + 23, sfCON::WindowHeight / 10 + 476), "Downaload"),
    sfC::Text_t ("../Fonts/baltimore-nouveau.ttf", 22, sf::Vector2f (sfCON::WindowWidth / 3 + 56, sfCON::WindowHeight / 10 + 627), "Create"),
    sfC::Text_t ("../Fonts/baltimore-nouveau.ttf", 30, sf::Vector2f (sfCON::WindowWidth / 3 + 71, sfCON::WindowHeight / 10 + 772), "Exit") };
////////////////////////////////////////

bool sfDATA::MainMenu_t::AnalyseMenu (sfC::Window& window)
{
    //CHANGE STATIC
...
bool sfDATA::MainMenu_t::ActionPress (sfC::Window& window , int press)
{
    switch (press)
    {
    case type_button_::GAME:
        MenuCars_t::MenuCars (window , MenuCars_t::TypeGame::Solo);
        return false;

    case type_button_::NETWORK:
        MenuCars_t::MenuCars (window , MenuCars_t::TypeGame::Network);
        return false;

    case type_button_::RESTART:
        Full_Restart ();
        sfF::ShowDoneAction (window , "Restarted");
        return true;
    ...
    default:
        LOG_warning << "Existing not done action!";
    }
    return false;
}

bool sfDATA::MainMenu_t::MainMenu (sfC::Window& window)
{
    ...
    while (window->isOpen ())
    {
        ...
        window.Draw (main_pic_ , buttons_ , texts_);
    }
...
sfDATA::MenuCars_t::TypeGame sfDATA::MenuCars_t::type_game_ = sfDATA::MenuCars_t::TypeGame::Solo;
...
bool sfDATA::MenuCars_t::AnalyseMenuCars (sfC::Window& window)
{
    sf::Event event;
    while (window->pollEvent (event))
    {
        if (event.type == sf::Event::MouseButtonPressed)
        {
            if (event.mouseButton.button == sf::Mouse::Left)
            {
                ...
                {
                    if (type_game_ == TypeGame::Solo)
                        MainGame_t::MainGame (window , sort1 , sort2);
                    else
                    {
                        if (NetworkGame {window}.Start (sort1) == false)
                            return false;
                    }
                    break;
                }
...
bool sfDATA::MenuCars_t::MenuCars (sfC::Window& window , TypeGame type_game)
{
    type_game_ = type_game;
    ...
    while (window->isOpen ())
    {
        ...
        window.Draw (main_pic , car1 , car2 , main_buttons , buttons , texts , levels);
        car1.rotate (0.2f);
        car2.rotate (0.2f);
    }
...
std::array<sfC::Text_t , 4> sfDATA::MainGame_t::texts = {
    sfC::Text_t (sfCON_FILE_Fonts_ttf, 35, sf::Vector2f (sfCON::WindowWidth * 0.508f, 30.f), "First car "),
...
bool sfDATA::MainGame_t::AnalyseGame (sfC::Window& window , float time)
{
    if (CheckWin ())
        return false;
...
bool sfDATA::MainGame_t::MainGame (sfC::Window& window , const sfC::ParamCar_t& sort1 , const sfC::ParamCar_t& sort2)
{
    //SO MANY NEEDS
    ...
    while (window->isOpen ())
    {
        ...
        window.AddArgs (map , tank1 , tank2);
        ...
        window.AddArgs (mid_strip , texts);
