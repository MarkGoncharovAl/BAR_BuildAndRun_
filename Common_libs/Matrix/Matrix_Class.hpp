    ...
    size_ ({ that.size_.row, that.size_.col })
{
    if (that.Empty ())
    {
        LOG_warning << ("Using Constructor of copy with null value!");
        return;
    }
...
{
    if (this->Empty ())
        LOG_error << ("Can't InitElems with not initialized matrix. INITIALIZE IT!");
    if (init.size () != size_.row)
        LOG_error << ("Can't initialize matrix - ROWS!");

    Type_t* cur_data = data_;
    for (auto row_iter = init.begin (); row_iter != init.end (); ++row_iter)
    {
        if (row_iter->size () != size_.col)
            LOG_error << ("Can't initialize matrix - COLS!");
...
void Matrix_t<Type_t>::Set (size_t row , size_t col , const Type_t& init)
{
    if (data_ == nullptr)
    {
        LOG_warning << ("Can't set in nullptr massive!");
        return;
    }
    if (row >= size_.row || col >= size_.col)
    {
        LOG_warning << ("Can't set in out_massive data!");
        return;
    }
...
SetterRaw_t::operator[](size_t num) const
{
    if (num >= size_)
    {
        LOG_warning << ("Tried to get away of masive COLUMN!");
        return Type_t {};
    }
    return data_[num];
}
...
Matrix_t<Type_t>::operator[](size_t num) const
{
    if (data_ == nullptr || num >= size_.row)
        LOG_error << ("Tried to get away from massive ROW");
...
void Matrix_t<Type_t>::operator+=(const Matrix_t<Type_t>& that)
{
    if (!data_ || !that.data_)
    {
        LOG_warning << ("Not defined matrixes!");
        return;
    }
    if (size_.row != that.size_.row || size_.col != that.size_.col)
    {
        LOG_warning << ("Matrixes have different sizes!");
        return;
    }
...
Matrix_t<Type_t>& Matrix_t<Type_t>::transpose ()&
{
    if (!IsSquare ())
    {
        LOG_warning << ("Transposing wasn't done!");
        return *this;
    }
...
std::ifstream& operator>>(std::ifstream& in , Matrix_t<T>& matrix)
{
    for (auto& elem : matrix)
    {
        ...
        if (!in.good ())
        {
            LOG_warning << ("Operator >> for matrix wasn't done peoprly!\n");
            return in;
        }
    }
